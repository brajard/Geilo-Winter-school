...
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Results Uni-variate example}
...
\end{lstlisting}
\begin{itemize}[<+->]
    \item \verb|n_estimators|: number of trees is the better (generally the larger is the better)
    \item   \verb|max_features|: number of feature to consider at each split. The default number is the total number of feature. A large value makes a a small bias (accuracy) but a bigger variance (risk of overfitting)
      \item \verb|min_samples_fit|: number of feature to consider at each split. The minimum value of 2 means that the tree is fully developed (small bias but great variance).
\end{itemize}

\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Determination of the parameters}
    \begin{itemize}[<+->]
        \item Parameters that are not optimized during the training are called \alert{hyper parameters}.
        \item They can be determined using a score on the \alert{validation dataset} or using a \alert{cross-validation} procedure.
        \item A convenient (but very costly) procedure in scikit-learn: \alert{gridsearch}.
    \end{itemize}
    \pause
    ~\\
    \centering {Example on notebook}
\end{frame}
...
\alert{Gradient descent algorithms based on $\partial L/\partial w$}
\end{block}
\column{.6\textwidth}
\begin{enumerate}[<+->]
...
\end{itemize}
1 Update is performed after N forward of the neural net.
\column{.5\textwidth}
...
\begin{itemize}
 \item Repeat N times:\\
    ...
    \end{enumerate}
\end{itemize}
N Updates are performed after N forward of the neural net.
...
\begin{itemize}
 \item Repeat B times:\\
    ...
    \end{itemize}
\end{itemize}
N//B updates are performed after N forward of the neural net\\
...
A convolutional layer is composed of $p$ convolution (size of layer) extracting $p$ features from the data.
\begin{figure}
\centering
    \includegraphics[width=.6\textwidth,trim={0 3.5cm 12.5cm 5cm},clip]{fig/L2/Cnn-layer.png}
\end{figure}
\pause
\alert{The size of the feature space is generally very big}
\end{frame}
